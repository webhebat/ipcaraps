...
class Petanasional_model extends CI_Model {
    ...
    public function getnilai($kode=null, $subgrupid=null, $tgl1 = null, $tgl2 = null){
        ...
        $sql="SELECT R.unitid, P.id_prov id, IFNULL(P.nama ,'Belum Diketahui') kategori, COUNT(R.id) jumlah FROM registrasi R LEFT JOIN provinsi P ON P.id_prov = R.id_prov WHERE 1 AND R.deleted = 'n' and P.id_prov = '$kode' $where "; 
        $result = $this->db->query($sql)->row();

        return $result;
    }

    public function getnilaimax($subgrupid, $tgl1, $tgl2){
        ...
        $sql="SELECT MAX(jml) as jmlmax
            FROM (SELECT COUNT(id_prov) as jml 
            FROM registrasi 
            WHERE 1 AND deleted = 'n' $where
            GROUP BY id_prov) as reg "; 

            $result = $this->db->query($sql)->row()->jmlmax;
    ...
    public function get_count_prov($idprov, $subgrupid=null,$tgl1=null,$tgl2=null){
        ...
        $sql="SELECT COUNT(id) as jml FROM registrasi WHERE 1 AND deleted = 'n' and id_prov = '$idprov' $where "; 
