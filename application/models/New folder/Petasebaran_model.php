...
class Petasebaran_model extends CI_Model {
    ...
    public function getdashboard($kategori,$subgrupid,$jtanggal,$tgl1,$tgl2){
        ...
        switch ($kategori) {
            ...
            case 'subgrup':
                  echo $sql ="SELECT S.id, IFNULL(S.subgrup,'Belum Diketahui') kategori, COUNT(R.id) jumlah 
                FROM registrasi R 
    ...
    public function getmarker($kategori=null,$subgrupid=null){
        ...
        switch ($kategori) {
            case 'kabupaten':
                $sql="SELECT r.id_kab,count(r.id_kab) as jml, k.nama, k.lat,k.lng FROM registrasi r join kabupaten k on k.id_kab = r.id_kab WHERE 1 AND r.deleted = 'n' AND r.unitid = '$unitid' $where GROUP by k.nama   ";
                break;
            default:
                $sql="SELECT r.id_prov,count(r.id_prov) as jml, p.nama, p.lat, p.lng FROM registrasi r join provinsi p on p.id_prov = r.id_prov WHERE 1 AND r.deleted = 'n' AND r.unitid = '$unitid' $where  GROUP by p.nama   ";
                break;
        }
