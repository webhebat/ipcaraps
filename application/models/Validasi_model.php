...
class Validasi_model extends CI_Model {
    ...
    public function getjmlvalidasi(){
        ...
        $query=$this->db->query("SELECT id FROM registrasi WHERE validate='y' AND deleted = 'n' $whereadm1");
        $query2=$this->db->query("SELECT id FROM registrasi where validate='n' AND deleted = 'n' $whereadm1");
    ...
    public function get_validasi($offset, $limit, $search,$warning,$jenis,$area,$tgl,$tgl2,$validate)
    {
        ...
        $sql="SELECT R.*,CONCAT(R.tempat_lahir, ', ', R.tgl_lahir) AS ttl, CASE WHEN R.jenis_kelamin = 'l' THEN 'Laki-laki' WHEN R.jenis_kelamin = 'p' THEN 'Perempuan' END AS jkelamin,S.kodesubgrup,S.subgrup,S.stagingid as idstaging,M.kodemorfologi,M.morfologi,T.kodetopografi,T.topografi,P.nama as propinsi,K.nama as kabupaten,U.nama_unit,CASE WHEN R.literalitas = 'l1' THEN 'Unilateral -kanan' WHEN R.literalitas = 'l2' THEN 'Unilateral -kiri' WHEN R.literalitas = 'l3' THEN 'Bilateral' END AS literalitas FROM registrasi R
            ...
            WHERE 1 AND R.deleted = 'n' $where $whereadm1 ORDER BY R.id DESC";
