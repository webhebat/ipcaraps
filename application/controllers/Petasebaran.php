...
class Petasebaran extends CI_Controller {
	...
	public function __construct()
    {
       ...
       /* $group = 'admin';
        ...
        $this->load->model('petasebaran_model');
    }

    public function index(){
        cek_session_admin();
        $this->load->view('petasebaran_view');
    }

    public function getmarker2(){
         $data = $this->petasebaran_model->getmarker2();
         $i  = 0;
         $rows   = array(); 
         foreach ($data as $r) {
             $rows[$i] = $r;
    ...
    public function getmarker(){
        ...
        $data = $this->petasebaran_model->getmarker($kategori,$subgrupid);
        $i  = 0;
        $rows   = array(); 
        foreach ($data as $r) {
            $rows[$i] = $r;
